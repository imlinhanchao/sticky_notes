...
<template>
  <li 
    class="relative m-2 cursor-pointer note-item rounded"
  >
    <div class="absolute w-full h-full border-2 border-current rounded hidden-lock" :class="{ 
    ...
    <div @click="data.finish = !data.finish" v-if="!data.editable" class="border-inherit" :class="{ 
      ...
      <section 
        class="absolute z-10 hidden text-xs toolbar right-2 bottom-[-20px]" 
      >
      <div class="absolute w-full h-full border-2 rounded-b pointer-events-none toolbar z-0" style="border-top-color: var(--background-color);"  :class="{ 'opacity-50': data.finish}"/>
      <div class="z-10 px-2 space-x-2 relative hidden-lock">
